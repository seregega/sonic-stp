...
def is_stp_docker_running():
    ...
    if running_docker.decode('utf-8').find('docker-stp') == -1:
       return False
    else:
       return True
...
def spanning_tree(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        for vlanid in vlan_list:
            ctx.invoke(show_stp_vlan, vlanid=vlanid, passedByGlobal=True)
...
@click.pass_context
def show_stp_vlan(ctx, vlanid, passedByGlobal=False):
    """Show spanning_tree vlan <vlanid> information"""
    global g_stp_vlanid
    g_stp_vlanid = vlanid

    global_stp_mode = get_global_stp_mode()
    if global_stp_mode == 'stp' and passedByGlobal == False:
        ctx.fail("VLAN related settings cannot be displayed when spanning-tree mode is STP.")
    ...
    click.echo("")
    if global_stp_mode == 'stp':
        click.echo("STP instance {}".format(vlan_tb_entry['stp_instance']))
    else:
    ...
    if ctx.invoked_subcommand is None:
        ...
        for po_name in po_list:
            ctx.invoke(show_stp_interface, ifname=po_name)
    ...
    for ifname in ifname_all:
        ...
        if entry['root_guard'] == 'true' and entry['enabled'] == 'true':
            if print_header:
                ...
                click.echo("")
                if get_global_stp_mode() == 'stp':
                    click.echo("{:17}{}".format("Port","Current State"))
                    click.echo("------------------------------------")
                else:
            ...
            if keys:
                for key in keys:
                    ...
                    if entry and 'root_guard_timer' in entry:
                        ...
                        if get_global_stp_mode() == 'stp':
                            click.echo("{:17}{}".format(ifname,state))
                        else:
...
def show_stp_statistics(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        vlan_list.sort()
        for vlanid in vlan_list:
            ctx.invoke(show_stp_vlan_statistics, vlanid=vlanid, passedByGlobal=True)


@show_stp_statistics.command('vlan')
@click.argument('vlanid', metavar='<vlanid>', required=True, type=int)
@click.pass_context
def show_stp_vlan_statistics(ctx, vlanid, passedByGlobal=False):
    ...
    if global_stp_mode == 'stp':
        click.echo("STP instance {}".format(stp_inst_entry['stp_instance']))
    else:
