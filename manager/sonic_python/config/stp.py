...
def vlan_enable_stp(db, vlan_name):
    fvs = {'enabled': 'true',
           'forward_delay': stp_common.get_global_stp_forward_delay(db),
           'hello_time': stp_common.get_global_stp_hello_time(db),
           'max_age': stp_common.get_global_stp_max_age(db),
           'priority': stp_common.get_global_stp_priority(db)
           }
    if get_stp_enabled_vlan_count(db) < stp_common.get_max_stp_instances():
...
def spanning_tree_enable(_db, mode):
    ...
    if mode != 'pvst':
        ...
        disable_spanning_tree(db)

        db.set_entry('XSTP_GLOBAL', 'Global',         {'value': 'enabled'})
        db.set_entry('XSTP_GLOBAL', 'mode',           {'value': mode})
        db.set_entry('XSTP_GLOBAL', 'forward-time',   {'value': XSTP_FORWARD_DELAY_DFLT})
        db.set_entry('XSTP_GLOBAL', 'hello-time',     {'value': XSTP_HELLO_INTERVAL_DFLT})
        db.set_entry('XSTP_GLOBAL', 'max-age',        {'value': XSTP_MAX_AGE_DFLT})

        members = [ v for k, v in db.get_table('VLAN_MEMBER')]
        members = list(dict.fromkeys(members))
        for member in members:
            db.set_entry('XSTP_INTERFACE', member, {'spanning-disable': 'disabled'})
    ...
    stp_common.enable_stp_for_interfaces(db)
    stp_common.enable_stp_for_vlans(db)
